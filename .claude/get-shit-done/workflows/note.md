...
- **Global scope**: `C:/VS Code/Akza/akza-client/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `C:/VS Code/Akza/akza-client/.claude/notes/`)
...
2. Glob `C:/VS Code/Akza/akza-client/.claude/notes/*.md` (if directory exists) — global notes
...
Global (C:/VS Code/Akza/akza-client/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `C:/VS Code/Akza/akza-client/.claude/notes/` — works in any directory
